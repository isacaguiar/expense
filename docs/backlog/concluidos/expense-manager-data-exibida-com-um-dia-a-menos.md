...
Confirmado na prática: a mesma função (`new Date(dateStr).toLocaleDateString(...)`) foi copiada para o esqueleto de `GroupSummary.tsx` (`docs/feature/20260818-resumo-grupo-dashboard/`) e, testado no browser local (fuso `America/Sao_Paulo`), exibiu "15 de jul." para um ciclo cujo início real era `2026-07-16` — corrigido nessa tela nova construindo a `Date` a partir dos componentes `year/month/day` em vez de parsear a string ISO diretamente. `ExpenseManager.tsx` não recebeu a mesma correção (fora do escopo desta feature, que só toca a tela de Resumo).
