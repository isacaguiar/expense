...
| **Memória** | O rastro de execução: log por task (comando executado + resultado real), índices de backlog e de bugfix, e as decisões arquiteturais. Não é memória "de sessão" — é rastro versionado. | `docs/feature/<AAAAMMDD>-<slug>/implementation.md` (concluída: `docs/feature/concluidas/<AAAAMM>/…`), `../backlog/README.md`, `../bugfix/README.md`, `decisions/` |
