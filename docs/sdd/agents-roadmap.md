...
| Checklist de concorrência de execução em `pr-readiness-checker` + `/code-review` (fonte única, `ADR-007`) | Backend — qualquer operação que leia e depois escreva um valor compartilhado (saldo, contador) sob concorrência | Quando existir, no repo, uma operação real desse tipo confirmada como sensível a concorrência — hoje nenhuma identificada. Nota de design: o gatilho de invocação não pode depender só de uma lista mantida manualmente (circular — se ninguém marcar a função como "sensível", a Skill nunca é chamada); resolver com sinal estrutural automático no diff (padrão leitura-then-escrita, não lista) + autodeclaração no PR (mesmo princípio do passo 4 do `pr-readiness-checker`: evidência explícita, não lista esquecida) — sinal sem declaração vira alerta pro humano, não bloqueio automático. |
