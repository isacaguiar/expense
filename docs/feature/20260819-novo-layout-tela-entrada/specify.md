...
> Feature: modernizar visualmente a tela "Resumo do grupo" (`GroupSummary.tsx`, rota `/groups/:id/summary` — primeira tela que o usuário vê após o login, via `/summary` → `SummaryEntry.tsx`), substituindo o layout atual — `AppBar` horizontal + `Container`/`Card`/`List` genéricos do MUI — por um layout com sidebar de navegação e cards de resumo, conforme mockup fornecido pelo usuário (`assets/images/entrada.png`). Pedido novo, sem épico correspondente em `docs/sdd/03-tasks.md`; reaproveita a paleta/identidade visual já criada na feature `docs/feature/20260819-novo-layout-tela-login/`. A feature anterior de dados desta tela (`docs/feature/20260818-resumo-grupo-dashboard/`) já implementou toda a lógica funcional e explicitamente deixou o redesenho visual "de fora do escopo" — é isso que esta feature resolve agora.
...
O mockup usa rótulos como "Este mês" (sob "Total de despesas") e "Despesas do mês". A tela real trabalha com "ciclo de fechamento" (que pode não coincidir com o mês calendário — ver `docs/feature/20260818-resumo-grupo-dashboard/specify.md` §R2). Copiar esses rótulos literalmente do mockup induziria a erro.
