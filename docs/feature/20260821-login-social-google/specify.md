...
- Se já existe um `ex_users` com esse e-mail (conta criada via e-mail/senha ou por outro meio), a conta é **vinculada automaticamente** ao `google_id` retornado (sem exigir senha nem confirmação adicional) — decisão tomada em 2026-08-21. **Correção de 2026-09-23 (achado da revisão de segurança, TASK-368):** a decisão original assumia "o Google só retorna e-mails verificados" sem checar isso de fato — o Socialite já expõe o campo `email_verified` do payload OIDC do Google (`$googleUser->getRaw()['email_verified']`), e não checá-lo permitia takeover de conta local por quem controlasse uma conta Google com e-mail não verificado igual ao de uma vítima já cadastrada. O auto-vínculo por e-mail agora só acontece se `email_verified` vier `true`; caso contrário, cai no mesmo redirect de erro (`/login?google_error=1`) — a conta existente não é tocada. Mesma regra de unicidade de `google_id` que o `intent=link` já aplica (um `google_id` não pode ficar associado a duas contas — hoje isso já é coberto por `GoogleAuthControllerTest::test_callback_redirects_with_error_when_google_id_already_linked_to_another_user`).
