...
- **TASK-314**: `grep -n "VITE_GA_MEASUREMENT_ID" .github/workflows/deploy-frontend.yml` mostra a variável no mesmo bloco `env:` do passo de build, ao lado de `VITE_API_BASE_URL`. Prova local de que o valor chega no bundle, sem depender do deploy: `cd frontend && VITE_GA_MEASUREMENT_ID=G-RNQM4DT19G npm run build && grep -rl "G-RNQM4DT19G" dist/` devolve pelo menos um arquivo.

- **TASK-315**: `npx tsc --noEmit` limpo. No preview, com o cookie ausente: `window.dataLayer` traz o `consent default` negado e `read_network_requests` mostra zero chamada ao Google. Chamando a função de aceite pelo console, a requisição a `gtag/js` acontece na hora e o cookie `_ga` passa a existir. Gravar `scd_consent=denied` à mão e recarregar mantém zero chamada.
