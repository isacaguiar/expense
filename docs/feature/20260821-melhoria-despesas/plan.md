...
- **Decisão tomada durante a implementação (TASK-174)**: não existia formulário de edição de despesa no frontend — `ExpenseView.tsx` era somente leitura (e ainda buscava via `GET /groups/{groupId}/expenses?year&month`, quebrado para qualquer despesa fora do mês calendário atual). Confirmado com o usuário: construir o modo de edição agora, não só apontar o ícone "Editar" para a tela de visualização. Isso virou task própria (`TASK-179`, ver `tasks.md`) em vez de inchar o escopo da TASK-174 — inclui trocar `ExpenseView.tsx` para `GET /expenses/{expenseId}` (rota de recurso único já existente no backend, correta independente de competência) e um modo de edição consumindo o `PUT /expenses/{id}` que o backend já tinha antes desta feature.
- **Achado durante a implementação (TASK-174)**: `GET /groups/{groupId}/expenses/summary` só devolvia nomes (`payerName`, `participants`), sem IDs — o frontend não tinha como decidir com segurança "o usuário logado é o credor" (pra mostrar o botão de marcar como paga) ou "é dono" (editar/excluir) sem comparar por nome, frágil. Virou `TASK-180`: adicionar `userPayerId`/`userCreatorId` na resposta de cada despesa em `computeCycleSummary()` — mudança aditiva, sem quebrar o formato existente.
