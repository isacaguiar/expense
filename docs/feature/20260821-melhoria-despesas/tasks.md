...
| TASK-150 | `ExpenseController::store` — forçar `paid=false` sempre na criação de quotas, ignorando valor enviado pelo cliente | backend | plan.md §6 | antes do merge | Pendente |
...
- **TASK-157**: `POST /groups/{groupId}/expenses/close` na competência vigente (ainda `open` por `BillingCycle`) cria/atualiza (upsert) o `GroupCycleSnapshot` daquela competência com `closed_manually_at = now()`, `totals`/`expenses`/`balances` recalculados, e materializa `Quota` para toda `FIXED` daquele mês que ainda não tinha; chamar de novo (re-fechar) sobrescreve o mesmo registro sem criar duplicata (`unique(group_id, cycle_start)` preservado). Fora da competência vigente (já `closed` automático), retorna 422.
