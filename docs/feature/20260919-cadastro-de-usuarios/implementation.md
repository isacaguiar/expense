...
| TASK-287 | Concluída | 2026-09-19 | Claude | `npx tsc --noEmit` exit 0; `npx vitest run src/pages/RegisterPage.test.tsx` — 13 passed. **Ponta a ponta real** (backend em :8000, Vite em :3000, Mailpit em :8025): formulário → e-mail com o código `747184` na caixa de `ana-e2e-b@example.com` → confirmação → `location.pathname` = `/app/meus-grupos` com JWT no `localStorage` e o shell autenticado renderizado. No banco: `whatsapp=(11) 98765-4321`, `role=user`, `email_verified_at` preenchido, `Hash::check` da senha original = SIM, `consumed_at` preenchido e `password`/`code_hash`/`handle_hash` vazios | A máscara de telefone foi conferida digitando no browser real (`11987654321` → `(11) 98765-4321`), não só no jsdom. Ficaram duas linhas de teste no banco local de dev (`ana-e2e-0919@example.com`, `ana-e2e-b@example.com`) — não removidas porque apagar é hard delete (`00-constitution.md` §5.2) e é banco local. |
| TASK-288 | Concluída | 2026-09-19 | Claude | `npx tsc --noEmit` exit 0; `npx vitest run src/pages/LoginPage.test.tsx src/pages/RegisterPage.test.tsx` — 19 passed. No browser: clicar em "Cadastre-se" no card de login leva a `/app/cadastro` **sem recarregar** (um marcador posto em `window` antes do clique sobreviveu). Site servido em :4173 — os 3 CTAs (`Cadastre-se`, `Começar agora é grátis`, `Criar minha conta grátis`) apontam para `/app/cadastro`, e `Entrar` segue em `/app/` | `LoginPage.test.tsx:128` fixava `href="#"` e foi atualizado junto, como previsto em `plan.md` §7. Sob `MemoryRouter` o href é `/cadastro`; no app real, com `basename="/app"`, vira `/app/cadastro`. |
| TASK-289 | Concluída | 2026-09-19 | Claude | `docs/sdd/01-specify.md` v1.0 → v1.1: §2 ganhou a linha `ex_user_pre_create`, §3.1 descreve as 3 rotas do auto-cadastro e explica o `handle`, §3.2 e §3.7 atualizados. Itens 042-046 criados em `docs/backlog/` e indexados no README | IDs começam em 042 porque o 041 foi ocupado por outra conversa em paralelo (rodapé do site) enquanto esta feature estava em andamento. |
