...
- **TASK-006**: no banco de produção, as tabelas `_fech_entries`, `_fech_owed`, `_fech_settlements` (trabalho) e `ex_expense`, `ex_expense_payers`, `ex_group`, `ex_user` (legadas do import de 2026-08-29, ausentes das migrations e do código) não existem mais; as 13 tabelas do sistema, `migrations` e os backups `_bkp_*` seguem intactas; e o app continua funcionando. Verificável pelas duas consultas de conferência do passo 4 do script (nenhuma constraint órfã, nenhuma das quatro remanescente) e pela navegação no app. Execução do SQL é do usuário — gate humano. O script é `cleanup-prod-tabelas-auxiliares.sql`.
