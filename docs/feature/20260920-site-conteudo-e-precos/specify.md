...
- **Definir o destino de publicação do site.** `.github/workflows/deploy-site.yml` tem `local-dir: ./` e `server-dir` comentado (ver `docs/bugfix/concluidos/202608/20260829-deploy-site-secrets-quebrado.md`), então o site não é publicado automaticamente. É decisão do dono e gate humano de deploy (`00-constitution.md` §5.2), não trabalho desta feature — mas sem isso nada daqui chega à produção.
