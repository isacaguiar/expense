...
- **R1**: Duas variantes de shell de página, ambas reaproveitando `brandColors` e **ambas com o mesmo menu lateral visual** (sidebar) do Resumo — correção pós-implementação: a primeira versão deste requisito previa cabeçalho simples sem sidebar para as rotas sem grupo, mas o usuário pediu que o menu apareça igual ao da tela de Resumo em toda página logada, não só dentro de um grupo:
  ...
  - **Sem grupo selecionado** (`Dashboard`/lista de grupos consolidada, `GroupForm` em criação, `ExpensesEntry`, `SummaryEntry`): mesmo componente de sidebar (generalizado para receber uma lista de itens em vez de depender de `groupId`), com itens genéricos que não dependem de um grupo específico — "Meus Grupos" (`/dashboard`), "Resumo" (`/summary`, entra no seletor), "Despesas" (`/expenses`, entra no seletor); "Participantes"/"Pagamentos"/"Configurações" ficam como placeholder (`href="#"`, mesmo tratamento visual que "Pagamentos"/"Relatórios" já têm dentro de um grupo), já que dependem de um grupo selecionado. Cabeçalho reaproveita o mesmo componente do shell "dentro de um grupo" (`GroupSummaryHeader`), sem seletor de grupo (não há grupo no contexto). Permanece fora de `InternalLayout`/`Navbar`, para não ter 3 sistemas de navegação diferentes no app.
