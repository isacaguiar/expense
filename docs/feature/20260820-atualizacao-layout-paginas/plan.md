...
- **`SimpleShellLayout`** (`frontend/src/layouts/SimpleShellLayout.tsx`, layout route com `<Outlet />`): **correção pós-TASK-090/091** — a primeira versão tinha só um cabeçalho simples, sem sidebar; o usuário reportou que a página `/dashboard` "não apresentou o menu" e pediu o mesmo menu do Resumo em toda página logada. Solução: extraí a renderização da sidebar de `GroupSidebar.tsx` para um componente genérico `frontend/src/layouts/Sidebar.tsx` (recebe `items: GroupNavItem[]`, sem depender de `groupId`); `GroupSidebar` virou um wrapper fino (`<Sidebar items={groupNavItems(groupId)} />`). `SimpleShellLayout` passa a montar `<Sidebar items={simpleNavItems()} />` (nova função em `frontend/src/layouts/simpleNavItems.ts`, itens: "Meus Grupos"→`/dashboard`, "Resumo"→`/summary`, "Despesas"→`/expenses`, "Participantes"/"Pagamentos"/"Configurações" como placeholder `href="#"`, mesmo tratamento visual dos itens desabilitados do `GroupSidebar`) + reaproveita o próprio `GroupHeader` (passando `groups=[]`/`groupId=''`, que já esconde o `Select` de grupo quando não há grupo — não precisou de um header novo), com `title` derivado do mesmo jeito que `GroupShellLayout` faz (busca em `simpleNavItems()` pelo item cujo `to` bate com `location.pathname`).
...
- Mesmo padrão das duas features anteriores: `GroupShellLayout` e `SimpleShellLayout` (após a correção do item 1, ambos usam o mesmo `Sidebar`) ocultam a sidebar abaixo do breakpoint `md` (mesma implementação, herdada de `GroupSummarySidebar`); o cabeçalho (`GroupHeader`, reaproveitado pelos dois shells) precisa não quebrar em telas estreitas (`flexWrap`, já implementado).
