...
| TASK-073 | Concluída | 2026-08-19 | IA (Claude) | `python -c "import xml.dom.minidom as m; m.parse('src/assets/illustrations/login-hero.svg')"` — sem erro (XML bem formado). `GET http://localhost:3000/src/assets/illustrations/login-hero.svg` via preview → `200 OK` (`read_network_requests`). | Arte vetorial original (formas geométricas simples: 3 pessoas estilizadas + notebook + 3 badges — checklist, cifrão, gráfico de pizza), sem referência a asset de terceiros. Screenshot visual não disponível neste ambiente; validação por XML bem-formado + carregamento HTTP 200. |
