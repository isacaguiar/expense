...
**Decisão de validação**: `act` (runner local de GitHub Actions) não está instalado nesta máquina (`which act` não encontrou o binário) — instalar só para esta validação seria ferramenta nova para um caso de uso único. A validação real aconteceu no próprio PR desta feature (#197) contra `dev`: abrir o PR já disparou `ci-backend.yml` de verdade. O plano original previa provar o caminho "falha quando deveria" com um commit de quebra proposital seguido de reversão — na prática, o próprio workflow revelou **dois bugs reais** nos primeiros runs (débito de Pint pré-existente em `dev`, TASK-356; e `.env` sem `APP_KEY=` para o `key:generate` substituir, causando `MissingAppKeyException`), cada um com seu próprio ciclo vermelho→correção→verde real, já registrado em `implementation.md`. Isso é evidência mais forte do que uma quebra artificial (prova que o check pega problema de verdade, não só um cenário fabricado) — a quebra proposital do plano original foi descartada por redundante.
