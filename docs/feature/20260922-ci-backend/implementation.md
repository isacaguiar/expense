...
| TASK-355 | Concluída | 2026-09-22 | Claude | Run final `35803223162` (commit `899a299da0`, fix do `APP_KEY`): `gh pr checks 197` → `Pint + PHPUnit (backend)` `pass` em 56s | Critério satisfeito por evidência real (decisão em `plan.md §2`), não pela quebra artificial planejada originalmente: os dois runs anteriores do próprio PR #197 já eram vermelhos por bugs genuínos (débito de Pint da TASK-356; `MissingAppKeyException` corrigido nesta task) — dispensa fabricar uma quebra só pra provar o óbvio |
