...
| TASK-028 | Mergeado (PR #6) | 2026-08-17 | Claude (IA) | `npx tsc --noEmit` — sem erros. Validado no browser (`npm run dev`, porta 3000): navegar para `/dashboard` sem `accessToken` em `localStorage` renderiza a tela de Login (`location.href` volta pra `/`); com `localStorage.setItem('accessToken', 'fake-token-for-guard-test')`, `/dashboard` renderiza "Meus Grupos" normalmente. | Branch/PR desta task ainda segue o fluxo antigo (direto pra `main`) — feature iniciada antes da convenção `branch → dev → main` (`00-constitution.md` §5.1). |
| TASK-029 | Mergeado (PR #5, em `dev`; reconciliado em `main`) | 2026-08-17 | Claude (IA) | `npx tsc --noEmit` — sem erros. Backend real rodando local (`php artisan serve --port=8000`, sem Docker/MySQL — middleware `jwt.auth` rejeita token inválido antes de tocar no banco: `curl -H "Authorization: Bearer garbage-invalid-token" http://localhost:8000/api/groups` → `401`). No browser (`npm run dev`, porta 3000, servido pelo usuário): com `accessToken=garbage-invalid-token` no `localStorage`, navegar para `/groups` chama `GET http://localhost:8000/api/groups` → `401 Unauthorized` (confirmado em `read_network_requests`) e redireciona pra `/` — comportamento novo confirmado ponta a ponta. `ExpenseManager` não pôde ser validado ponta a ponta pelo mesmo caminho: achado durante o teste (item 006 do backlog) mostra que `groupId` fica sempre `undefined` (nome de param errado) e que o endpoint `/api/groups/{id}/expenses` nem existe no backend — o código da TASK-029 em `ExpenseManager.tsx` segue exatamente o mesmo padrão já validado em `GroupList.tsx` (mesmo `if (err.response?.status === 401) navigate('/', ...)`), mas a chamada de rede real nunca dispara hoje por causa desse bug pré-existente e não relacionado. | Achados tangenciais registrados como `docs/backlog/006` (ExpenseManager nunca carrega despesas) e `docs/backlog/007` (cache/log do Laravel versionados). |
