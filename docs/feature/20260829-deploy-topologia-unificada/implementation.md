...
| TASK-231 | Concluída | 2026-08-29 | IA (branch `frontend/...-TASK-231`) | `npx tsc --noEmit` limpo · `npm run build` OK (`dist/index.html` → `/app/assets/index-*.js`, `dist/.htaccess` com `RewriteBase /app/`) · `npx vitest run` → 26 arquivos / 174 passed (1 flake numa 1ª execução — `AcceptInvitePage` isolado 3/3, usa MemoryRouter; verde na confirmação) | `vite.config.js` `base: '/app/'`; `main.tsx` `<BrowserRouter basename="/app">`; `App.tsx` import `BrowserRouter as Router` removido (era morto); `frontend/public/.htaccess` `RewriteBase /app/` + fallback `/app/index.html`; `deploy-frontend.yml` `server-dir` `/frontend/` → `/www/app/`. Nenhum teste precisou de ajuste (usam MemoryRouter). |
