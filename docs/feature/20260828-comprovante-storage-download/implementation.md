...
| TASK-223 | Concluída | 2026-08-28 | IA (branch `frontend/...-TASK-223`) | `cd frontend && npx tsc --noEmit` limpo · `npx vitest run` → 26 arquivos / 174 passed | Código de app: nenhuma mudança — `Payments.tsx`, `ExpenseManager.tsx`, `ExpenseView.tsx`, `PayableSettlementList.tsx` já fazem `href={proofUrl}` verbatim, sem concatenar base URL. Só trocados os literais de mock em `ExpenseManager.test.tsx` e `ExpenseView.test.tsx` que usavam `http://localhost/storage/...` por um valor opaco (`/api/groups/1/proofs/quota/N?signature=stub`) — nenhum teste depende mais do formato da URL. |
| TASK-224 | Concluída | 2026-08-29 | IA (branch `doc/...-TASK-224`) + gate humano (diff do §6.5 aprovado pelo dono na conversa) | edição de docs, sem comando de validação | `ADR-005-download-arquivo-signed-url.md` criado (exceção estrita ao §6.5 para download de arquivo por signed URL); `00-constitution.md` §6.5 ganhou o parágrafo da exceção, `Versão: 1.2 → 1.3` / data 2026-08-29; `decisions/README.md` indexa ADR-005. |
