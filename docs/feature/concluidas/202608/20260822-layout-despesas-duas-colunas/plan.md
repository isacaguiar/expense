...
- `ExpenseManager.tsx:353` (`<Grid size={{ xs: 12, md: 8 }}>`) e `:484` (`<Grid size={{ xs: 12, md: 4 }}>`) trocam para `{ xs: 12, sm: 12, lg: 8 }` e `{ xs: 12, sm: 12, lg: 4 }` — exatamente o par já usado em `GroupSummary.tsx:134,173` (`docs/feature/concluidas/202608/20260822-reestruturacao-resumo/plan.md §1`), mesma razão já registrada lá: tablets estreitos (600-1200px) ficam empilhados junto com mobile em vez de espremer duas colunas.
...
- `ExpenseManager.tsx:484-489` (`<Typography>Saldo por pessoa</Typography>` + `<BalanceCards balances={summary.balances} />`) é substituído por `<SummarySidePanel balances={summary.balances} settlements={summary.settlements} cycleStatus={summary.cycle.status} />` — mesmo componente de `docs/feature/concluidas/202608/20260822-reestruturacao-resumo/`, já em `dev`, sem alteração nele.
