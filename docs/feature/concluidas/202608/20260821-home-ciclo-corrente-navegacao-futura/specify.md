...
> `docs/feature/concluidas/202609/20260902-pagamento-ciclo-fechado/` (§2.4, endpoint `focus-cycle`) passou a
...
**Importante**: isso reverte um requisito explícito da feature original que criou essa tela — `docs/feature/concluidas/202608/20260818-resumo-grupo-dashboard/specify.md` R3: *"A tela 'Resumo do grupo' abre, por padrão, no ciclo fechado mais recente do grupo selecionado... (não no ciclo em andamento)"* — e `plan.md` (linha ~23, ~39): *"o ciclo que contém hoje é 'em andamento' e nunca é o default"*, *"nunca navega para o ciclo em andamento"*. Não é correção de bug; é mudança de requisito de produto, decidida nesta conversa.
