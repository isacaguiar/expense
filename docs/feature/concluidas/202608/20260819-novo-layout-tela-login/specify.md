...
- **Internacionalização real**: o seletor "Português (Brasil)" / "English (US)" é um `Select` do MUI funcional como componente (abre, permite escolher), mas sem nenhuma lógica de tradução associada — não há sistema de i18n no `frontend/` hoje e esta feature não o implementa. Registrado como ideia de backlog `016` em `docs/backlog/`.
