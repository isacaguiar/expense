...
- Botão "Editar" desabilitado (com `Tooltip` explicando o motivo) quando `expense.expense_type === 'IN_INSTALLMENTS'` e `expense.quotas.some(q => q.paid)` — mesmo dado que a feature anterior (`docs/feature/concluidas/202608/20260825-pagamentos-grid-pix/`) já passou a carregar.
