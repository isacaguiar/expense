...
`frontend/src/components/RequireAuth.tsx` (guard de rota, `TASK-028`) e a lógica de redirect em 401 de `GroupList.tsx`/`ExpenseManager.tsx` (`TASK-029`) foram implementados nesta mesma sessão sem nenhum teste automatizado — hoje a única validação é manual, feita no browser a cada mudança. São bons primeiros alvos por serem unidades pequenas, isoladas e já terem comportamento conhecido/documentado (`docs/feature/concluidas/202608/20260817-config-url-api-frontend/`).
