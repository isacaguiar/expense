...
|---|---|---|---|---|---|
| TASK-189 | Gerar token de convite dedicado (TTL 2 dias) em `GroupMemberController::store`, substituindo `Password::getRepository()` | backend | plan.md §3 | antes do merge | Pendente |
| TASK-190 | Corrigir `UserInvitedMail` para renderizar `email.invitation` (em vez da view inexistente) citando quem convidou e o grupo | backend | plan.md §2 | antes do merge | Pendente |
