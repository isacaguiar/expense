...
|---|---|---|---|---|---|
| TASK-030 | Instalar e configurar Vitest + Testing Library no frontend web (`vite.config.js`, `setupTests.ts`, `tsconfig.json`, scripts `test`/`test:watch`) | frontend | plan.md §1 | nenhum | PR aberto |
| TASK-031 | Criar `RequireAuth.test.tsx` cobrindo os dois casos (sem token → login; com token → conteúdo protegido) | frontend | plan.md §2 | nenhum | PR aberto |
