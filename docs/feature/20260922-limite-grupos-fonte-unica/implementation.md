...
| TASK-353 | Concluída | 2026-09-22 | Claude (agent `security-reviewer`) | `git diff dev...backend/20260922-limite-grupos-fonte-unica -- backend/` revisado pelo agent — sem achado bloqueante. Confirmado: (1) `toArray()` produz o mesmo conjunto de campos que a serialização anterior (`Model::jsonSerialize()` já chama `toArray()` internamente), `$hidden` continua respeitado (`UserPhotoTest.php:40` prova isso); (2) `max_groups_per_user` é constante de código, não deriva de input nem de outro registro, e já era descobrível pela mensagem de erro de `GroupController::store`; (3) nenhum teste faz `assertExactJson` em `/api/me` | 1 achado informativo (não bloqueante, fora do diff desta feature) virou item de backlog [060](../../backlog/group-controller-log-info-user-completo.md) (`GroupController::store` loga o `User` inteiro no log) |
